...
class Profile extends Component {
  ...
  componentDidMount() {
    const x = true;
    if (x)
      axios.get("/bloodrequest/showexistingrequests",{withCredentials:true}).then((response) => {
  ...
  deleThisReq(code){
    const obj={code:code};
    axios.post("/bloodrequest/deletebycode",obj)
    .then(response=>{
    ...
    return (
      <div>
        ...
        <Table>
          ...
          {this.state.reqs&&this.state.reqs.map((data,key)=>{
            ...
            return(<tr>
              ...
              <td onClick={()=>{this.deleThisReq(data.code)}}>D</td>
            </tr>)
          })}
        </Table>
      </div>
